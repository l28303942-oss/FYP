...
class GTAFM(nn.Module):
    def __init__(self, c):
        ...
        self.net = nn.Sequential(
            nn.Conv3d(c, c, 3, padding=1), nn.ReLU(),
...
# =========================
# 5. MAIN TRAINING LOOP (UPDATED FOR RESUME)
# =========================
def train():
    ...
    # FIX: Batch size 4 aur Workers 8 (RTX 2060 Super handle kar lega)
    train_loader = DataLoader(train_ds, batch_size=4, sampler=sampler, num_workers=8, pin_memory=True)
    val_loader = DataLoader(PEBalancedDataset(val_path), batch_size=4, shuffle=False)

    model = TS_Attention_UNet().to(device)
    
    # FIX: LR 1e-5 for fine-tuning clots
    optimizer = optim.AdamW(model.parameters(), lr=1e-5, weight_decay=1e-5)
    
    # ---------- RESUME LOGIC START ----------
    checkpoint_path = "best_tsnet_model.pth"
    best_dice = 0
    
    if os.path.exists(checkpoint_path):
        print(f"\n--- Loading saved model from {checkpoint_path} ---")
        # Map location use kiya hai taake device mismatch na ho
        checkpoint = torch.load(checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint)
        print("Model Resume ho gaya! Purani mehnat barkarar hai.")
        # Note: Purana best_dice hum 0.6060 hi farz karenge jo save hua tha
        best_dice = 0.6060 
    else:
    ...
    warmup_sched = LinearWarmupScheduler(optimizer, warmup_epochs, 1e-5) # Adjusted to new LR

    criterion = DiceFocalLoss()
    scaler = torch.amp.GradScaler("cuda")
    
    # FIX: Patience 50 for longer training
    patience = 50
    counter = 0
    accumulation_steps = 4 # Batch size 4 + Accumulation 4 = Effective Batch 16 (Safe & Fast)
    ...
    for epoch in range(epochs):
        ...
        # Early Stopping Logic (Updated with 50 Patience)
        if avg_dice > best_dice:
            best_dice = avg_dice
            counter = 0
            torch.save(model.state_dict(), "best_tsnet_model.pth")
