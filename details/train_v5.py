...
class PEBalancedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # UPDATE: Added .copy() to remove UserWarnings
...
# ... [Architecture & Loss classes same rahein gi] ...
# (TAD, GTAFM, TS_Attention_UNet, DiceFocalLoss, LinearWarmupScheduler - Paste them here)
...
def train():
    ...
    # --- 1. SET CHECKPOINT ---
    checkpoint_file = "model_dice_0.5279_epoch_25.pth" # EXACT name check kar lein folder mein
    ...
    # UPDATE: Batch Size 2 and num_workers 2 (Better for HDD + RTX 2060)
    train_loader = DataLoader(train_ds, batch_size=2, sampler=sampler, num_workers=2, pin_memory=True)
    val_loader = DataLoader(PEBalancedDataset(val_path), batch_size=2, shuffle=False)
    ...
    if os.path.exists(checkpoint_file):
        ...
        print(f"✅ SUCCESS: Loaded checkpoint '{checkpoint_file}'. Continuing from here...")
    else:
        print("⚠️ WARNING: Checkpoint not found! Starting from scratch.")
    ...
    # --- 3. ADJUST BEST DICE ---
    best_dice = 0.5279 # Taake is se behtar hi save ho
    patience = 70; counter = 0
        ...
        if avg_dice > best_dice:
            ...
            if len(saved_models) > 10:
                ...
                if os.path.exists(worst_file):
                    ...
                    print(f"🗑️ Noise Removed: {worst_file}")
        else:
            counter += 1
            if counter >= patience:
